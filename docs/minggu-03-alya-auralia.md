...
Jawaban=<Br>
Setelah menghapus `$table->unique(['course_id', 'user_id'])` dari migration `create_course_user_table` lalu menjalankan `php artisan migrate:fresh --seed` untuk menerapkan perubahan tersebut. 
Kemudian selanjutnya mendaftarkan mahasiswa yang sama `(user_id = 5)` ke mata kuliah yang sama `(course_id = 1)` sebanyak dua kali secara manual melalui tinker menggunakan `attach()`.
...
Jawaban=<Br>
Untuk mensimulasikan kondisi di mana properti mass assignment protection tidak diterapkan dengan benar, dilakukan penambahan kolom `role` ke dalam `$fillable` pada model `User`. 
Setelah itu dibuat data user baru melalui tinker menggunakan `User::create()` dengan menyisipkan `role => admin`, seolah-olah ini adalah request dari form registrasi publik yang seharusnya hanya berisi `name`, `email`, dan `password`.
...
Jawaban=<Br>
Pertama kita mengganti`$fillable` pada model `User` dengan `protected $guarded = [];` untuk mensimulasikan kondisi di mana tidak ada satu pun kolom yang dilindungi dari mass assignment. 
Setelah itu mengulang nomor 2 yaitu dibuat data user baru melalui tinker menggunakan `User::create()` dengan menyisipkan seluruh kolom sekaligus, termasuk `role => admin` dan `id => 999` yang seharusnya tidak pernah bisa ditentukan manual oleh pengguna.
...
### 4. Kosongkan isi down() di satu migrasi, lalu jalankan php artisan migrate:refresh<Br>
Jawaban=<Br>
Pada migration `create_courses_table`, method `down()` sengaja dikosongkan untuk menguji apakah migration dapat di-rollback dengan benar. Perubahan yang dilakukan:
...
Jawaban=<Br>
Pada migration `create_courses_table`, `lecturer_id` yang seharusnya menggunakan `restrictOnDelete()` diubah menjadi:
...
mata kuliah yang memiliki `lecturer_id` tersebut juga ikut terhapus.

**Hasil pengamatan:**

Penggunaan `cascadeOnDelete()` menyebabkan penghapusan data induk (`users`) secara otomatis menghapus data anak (`courses`) yang memiliki foreign key ke dosen tersebut. Hal ini menunjukkan adanya kehilangan data secara berantai.

**Kesimpulan:**

`cascadeOnDelete()` tidak tepat digunakan pada `courses.lecturer_id` karena mata kuliah tidak seharusnya ikut terhapus ketika seorang dosen dihapus. Untuk KampusLMS, `lecturer_id` harus menggunakan `restrictOnDelete()` sehingga dosen tidak dapat dihapus selama masih mengampu mata kuliah.
